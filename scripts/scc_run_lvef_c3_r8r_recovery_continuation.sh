...
case "$JOB_NAME" in
  ...
  lvef_c3_r8u_r7h_fin_*)
    ...
    PYCACHE_ROLE=lvef_c3_r8u_r7h
    ;;
  lvef_c3_r8u_r7ha_ctx_*)
    [[ "${SGE_TASK_ID:-}" == "17" ]] || exit 78
    [[ "${NSLOTS:-}" == "1" ]] || exit 78
    export CUDA_VISIBLE_DEVICES=''
    JOB_FAMILY=r8u_r7ha
    ROLE=r8u_r7ha_context_probe
    MODE=probe
    CONTROLLER="$WORKTREE/scripts/lvef_c3_r8u_r7h_auth_successor.py"
    PYCACHE_ROLE=lvef_c3_r8u_r7ha
    ;;
  lvef_c3_r8u_r7ha_seq_*)
    [[ "${SGE_TASK_ID:-}" =~ ^1[7-9]$ ]] || exit 78
    [[ "${NSLOTS:-}" == "4" ]] || exit 78
    [[ -n "${CUDA_VISIBLE_DEVICES:-}" ]] || exit 78
    JOB_FAMILY=r8u_r7ha
    ROLE="r8u_r7ha_array_task_${SGE_TASK_ID}"
    MODE=array
    CONTROLLER="$WORKTREE/scripts/lvef_c3_r8u_r7h_auth_successor.py"
    PYCACHE_ROLE=lvef_c3_r8u_r7ha
    ;;
  lvef_c3_r8u_r7ha_fin_*)
    [[ "${SGE_TASK_ID:-undefined}" == "undefined" ]] || exit 78
    [[ "${NSLOTS:-}" == "4" ]] || exit 78
    export CUDA_VISIBLE_DEVICES=''
    JOB_FAMILY=r8u_r7ha
    ROLE=r8u_r7ha_finalizer
    MODE=finalizer
    CONTROLLER="$WORKTREE/scripts/lvef_c3_r8u_r7h_auth_successor.py"
    PYCACHE_ROLE=lvef_c3_r8u_r7ha
    ;;
