...
set -euo pipefail
umask 077
trap 'status=$?; if (( status != 0 )); then printf "%s\n" "D3_TRACKED_RECOVERY=FAILED" "D3_TRACKED_RECOVERY_ERROR=WRAPPER_AUTHORITY_FAILURE"; fi' EXIT
...
unset GIT_ALTERNATE_OBJECT_DIRECTORIES PYTHONPATH PYTHONHOME PYTHONINSPECT PYTHONSTARTUP
...
exec "$ECHOPRIME_PYTHON" -E -s \
  "$WORKTREE/scripts/finalize_lvef_phase1ef_d3.py" \
  "$1"
