#!/usr/bin/env python3
"""Body-free lock-time scheduler and process quiescence for R8U-R7C.

This observer is deliberately separate from every live continuation-worker
validator.  It captures one full XML scheduler snapshot and one process-table
snapshot, projects only control-plane metadata, and proves that no relevant
LVEF C3 execution remains active.  It does not inspect worker-role environment
variables and exposes no scientific execution path.
"""
...
SCHEDULER_COMMAND: Final = (
    str(QUERY_TOOL_PATH),
    "-xml",
    "-u",
    OWNER,
)
PROCESS_COMMAND: Final = (
    str(PROCESS_TOOL_PATH),
    "-axo",
    "pid=,uid=,command=",
)
...
# These are execution-only markers used throughout the immutable C3 workers.
# The R7C observer itself has none of them, so it is not silently exempted by
# PID and no active scientific/control worker can masquerade as the observer.
...
class R7CQuiescenceError(RuntimeError):
    """One stable fail-closed quiescence error."""
...
def _fail(code: str) -> None:
    raise R7CQuiescenceError(code)


@dataclass(frozen=True)
class FixedQuiescenceEvidence:
    """The lock projection and its two independently hashed source projections."""
...
def _argv_sha256(argv: tuple[str, ...]) -> str:
    return metadata.canonical_json_sha256({"argv": list(argv)})
...
def _validated_environment(environment: Mapping[str, str]) -> dict[str, str]:
    if not isinstance(environment, Mapping) or not environment:
        _fail("R8U_R7C_QUIESCENCE_ENVIRONMENT_INVALID")
    ...
    if (
        normalized.get("USER") != OWNER
        or normalized.get("LOGNAME") != OWNER
        or normalized.get("SGE_ROOT")
        != str(QUERY_TOOL_PATH.parents[2])
...
def _require_observer_tools() -> None:
    ...
    if QUERY_TOOL_PATH != expected or not QUERY_TOOL_PATH.is_absolute():
        _fail("R8U_R7C_QUIESCENCE_QUERY_TOOL_INVALID")
    ...
    except R7CQuiescenceError:
        raise
    except OSError as exc:
        raise R7CQuiescenceError(
            "R8U_R7C_QUIESCENCE_QUERY_TOOL_INVALID"
        ) from exc
    if (
    ...
    try:
        ...
        raise R7CQuiescenceError(
            "R8U_R7C_QUIESCENCE_PROCESS_TOOL_INVALID"
        ) from exc
    if (
        ...
        _fail("R8U_R7C_QUIESCENCE_PROCESS_TOOL_INVALID")
...
def _query_payload(
    ...
    *,
    ...
    try:
        ...
        raise R7CQuiescenceError(failure_code) from exc
    if completed.returncode != 0 or error or len(payload) > maximum_bytes:
        _fail(failure_code)
    return payload


def project_fixed_scheduler_snapshot(payload: bytes) -> dict[str, Any]:
    """Normalize one full XML snapshot without relying on display columns."""
    ...
    if (
        ...
        _fail("R8U_R7C_SCHEDULER_SNAPSHOT_INVALID")
    try:
    ...
    for job in root.iter():
        ...
        if any(
            ...
            for key in ("JB_job_number", "JB_name", "state")
        ):
            _fail("R8U_R7C_SCHEDULER_SNAPSHOT_INVALID")
        if len(fields.get("JB_owner", ())) > 1 or len(fields.get("tasks", ())) > 1:
            _fail("R8U_R7C_SCHEDULER_SNAPSHOT_INVALID")
        job_id = fields["JB_job_number"][0]
        name = fields["JB_name"][0]
        state_value = fields["state"][0]
        category = str(job.get("state", ""))
        owner = fields.get("JB_owner", [OWNER])[0]
        ...
        fixed_target_id = job_id in {ARRAY_JOB_ID, FINALIZER_JOB_ID}
        related_prefix = name.startswith(RELATED_OR_TRUNCATED_PREFIXES)
        if related_prefix and not exact_name:
            _fail("R8U_R7C_SCHEDULER_IDENTITY_AMBIGUOUS")
        if fixed_target_id:
            expected_name = (
                ARRAY_JOB_NAME if job_id == ARRAY_JOB_ID else FINALIZER_JOB_NAME
            )
        ...
        target_array_present = target_array_present or job_id == ARRAY_JOB_ID
        target_finalizer_present = target_finalizer_present or job_id == FINALIZER_JOB_ID
    ...
    if set(value) != SCHEDULER_PROJECTION_KEYS:
        _fail("R8U_R7C_SCHEDULER_PROJECTION_INVALID")
    return value


def _is_relevant_process(command: str) -> bool:
    return any(marker in command for marker in RELEVANT_PROCESS_MARKERS) or (
        RELEVANT_JOB_NAME_IN_COMMAND_RE.search(command) is not None
    )


def project_fixed_process_snapshot(
    payload: bytes, *, effective_uid: int
) -> dict[str, Any]:
    """Normalize a process-table snapshot using numeric owner identity."""
    ...
    if (
        ...
        _fail("R8U_R7C_PROCESS_SNAPSHOT_INVALID")
    try:
        lines = payload.decode("utf-8", "strict").splitlines()
    except UnicodeError as exc:
        raise R7CQuiescenceError("R8U_R7C_PROCESS_SNAPSHOT_INVALID") from exc
    ...
    for raw_line in lines:
        ...
        try:
            ...
            raise R7CQuiescenceError(
                "R8U_R7C_PROCESS_SNAPSHOT_INVALID"
            ) from exc
        command = fields[2]
        if pid <= 0 or uid < 0 or pid in seen_pids or any(
    ...
    value: dict[str, Any] = {
        "schema_version": 1,
        "artifact_type": "lvef_c3_r8u_r7c_process_quiescence_projection_v1",
        "status": (
    ...
    if set(value) != PROCESS_PROJECTION_KEYS:
        _fail("R8U_R7C_PROCESS_PROJECTION_INVALID")
    return value
...
def build_fixed_scheduler_state(
    ...
    """Build the exact scheduler-state projection consumed by the lock schema."""

    if not isinstance(scheduler_projection, Mapping) or not isinstance(
        process_projection, Mapping
    ):
        _fail("R8U_R7C_QUIESCENCE_PROJECTION_INVALID")
    ...
    if (
        ...
        != "lvef_c3_r8u_r7c_scheduler_quiescence_projection_v1"
        or scheduler_projection.get("scheduler_username") != OWNER
        ...
        != "lvef_c3_r8u_r7c_process_quiescence_projection_v1"
    ...
    if (
        ...
        _fail("R8U_R7C_ACTIVE_EXECUTION_CONTRADICTION")
    result = {
...
def _default_effective_uid() -> int:
    try:
        effective_uid = os.geteuid()
        account = pwd.getpwuid(effective_uid)
    except (KeyError, OSError) as exc:
        raise R7CQuiescenceError(
            "R8U_R7C_QUIESCENCE_IDENTITY_INVALID"
        ) from exc
    if account.pw_name != OWNER:
        _fail("R8U_R7C_QUIESCENCE_IDENTITY_INVALID")
...
def capture_fixed_quiescence(
    *,
    ...
    """Capture exactly one scheduler and one process snapshot for the R7C lock."""

    sealed_environment = _validated_environment(environment)
    try:
        tool_validator()
    except R7CQuiescenceError:
        raise
    ...
    observed_uid = (
        _default_effective_uid() if effective_uid is None else effective_uid
    )
    if (
    ...
    scheduler_payload = _query_payload(
        ...
        failure_code="R8U_R7C_SCHEDULER_QUERY_FAILED",
    )
    process_payload = _query_payload(
        ...
        failure_code="R8U_R7C_PROCESS_QUERY_FAILED",
    )
    scheduler_projection = project_fixed_scheduler_snapshot(scheduler_payload)
    process_projection = project_fixed_process_snapshot(
        process_payload, effective_uid=observed_uid
    )
    scheduler_state = build_fixed_scheduler_state(
        scheduler_projection, process_projection
    )
