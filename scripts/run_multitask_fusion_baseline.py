#!/usr/bin/env python3
"""Run multitask fusion baselines (vision embeddings + tabular features).

For each target task__* column:
- target = current task column
- features = [study embedding, all other task__* columns]
...
def main() -> int:
    ...
    if len(task_cols) < 2:
        raise RuntimeError("Need at least 2 task columns for leave-one-task-out fusion baseline.")
    ...
    x_vis = emb[joined[idx_col].to_numpy(dtype=int)]
    ...
    for task in task_cols:
        ...
        x_tab = joined[feature_cols].to_numpy(dtype=np.float32)
        x = np.concatenate([x_vis, x_tab], axis=1)
        ...
        row_base = {
            "task_col": task,
            "n_vision_features": int(x_vis.shape[1]),
            "n_tabular_features": int(len(feature_cols)),
        ...
        if "subject_id" in joined.columns:
            pred_task_cols = ["subject_id"] + pred_task_cols
        pred_task = joined[pred_task_cols].copy()
    ...
    summary = {
        ...
        "n_joined_subjects": int(joined["subject_id"].nunique()) if "subject_id" in joined.columns else None,
    ...
    metrics_csv = args.output_dir / "multitask_fusion_task_metrics.csv"
    predictions_csv = args.output_dir / "multitask_fusion_predictions_long.csv"
    summary_json = args.output_dir / "multitask_fusion.summary.json"
