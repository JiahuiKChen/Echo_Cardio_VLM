...
The first production-lock environment capture is preserved as a failed,
pre-root attempt: the unchanged EchoPrime Python lacks `google-crc32c`. The
executable lock now binds a separate, isolated Cloud-SDK-bundled Python 3.14
CRC32C worker—interpreter, compiled distribution tree, protocol, known vector,
and tracked worker are all checksummed—without installing or altering either
runtime.
