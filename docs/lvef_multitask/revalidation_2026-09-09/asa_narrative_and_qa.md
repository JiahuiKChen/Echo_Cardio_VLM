...
| Why exclude five studies from structured-only? | “The primary scientific question is paired modality comparison on identical imaging-eligible subjects. A structured-only full-availability analysis would have a separate denominator. It is not activated or reported in the current run, which excludes the five no-cine studies from all modalities.” |
