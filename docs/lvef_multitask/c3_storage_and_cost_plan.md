...
## SCC quota charge discrepancy

Current BU documentation states `$22/TB/year`, a six-month minimum, and annual billing prorated for shorter periods. That arithmetic gives **$11 for one additional TB for six months**, not $132. The owner supplied `$132` as the intended six-month incremental charge; `$132` equals six months at `$22/TB/month`.

The quota request may preserve `$132` only as an **owner-reported authorization ceiling pending RCS confirmation**. It must not present `$132` as a calculation from `$22/TB/year`. The billing unit is a pre-purchase administrative gate, not a scientific gate.

## Copy-ready quota request

> Please reallocate the mimicecho project's 200-GB restricted backed-up baseline allocation into `/restricted/projectnb` if RCS confirms that the exchange is supported, and add one 1-TB Storage-as-a-Service increment to `/restricted/projectnb`. The intended final research-tier quota is 2,000 GB. The project will retain the selected 4,530-study raw DICOM source set and use a one-batch rolling extracted-clip cache; it will not mirror all 7,243 public studies. The exact C3 peak and headroom will be attached from the metadata-only source preflight before transfer authorization. We currently carry an owner-reported six-month budget ceiling of $132, but BU's published $22/TB/year prorated rate implies $11 for one TB over the six-month minimum; please confirm the applicable charge in writing before the request is finalized. Before any full exchange of backed-up space, we will checksum, migrate/recreate, and recovery-test the shared Git worktrees and preserve irreplaceable restricted authorities in an approved disaster-recovery location.
