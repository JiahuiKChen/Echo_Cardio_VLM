...
`phase1ebc_autoclass_adjudication_attempt_001` binds the six immutable job-7104307 outputs, one owner-private request receipt, one owner-private exact raw response, one owner-private response receipt, the primary-source registry, corrected code and policy hashes, and six aggregate-safe products. Creation is exclusive and no-clobber. The pre-request receipt is written before network access, making the attempt non-rerunnable even after a partial failure.
