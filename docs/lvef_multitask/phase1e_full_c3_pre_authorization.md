...
| Requester-pays budget approved | `ABSENT` | Owner/financial approval for exact preflight-derived estimate plus contingency | Blocks body transfer |
...
| SCC quota charge confirmed | `BLOCKED_ADMIN_RATE_CONTRADICTION` | RCS resolves $22/TB/year versus owner-supplied $132/6-month charge | Blocks quota purchase, not scientific design |
...
Retaining 50 GB of backed-up quota is only a provisional planning option if partial exchange is supported; it is not yet a sufficiency finding. The final retained and migrated amounts require a complete checksum-bound path classification. Full reallocation is permitted only after all irreplaceable restricted authorities have a separately approved checksum-verified backup and both worktrees pass a recovery test.
...
2. exact raw bytes, final 2-TB peak, and requester-pays budget approval;
3. verified backup/migration readiness and final quota confirmation;
...
5. frozen enriched source, command, config, checkpoint, and environment identities;
