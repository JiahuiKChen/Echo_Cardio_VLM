...
The subsequent Phase 1C SCC execution is recorded in `phase1c_scc_execution_findings.md`. Its completed provenance and exact-threshold outputs remain valid; the clinical metadata packet was not generated because the command selected an unsupported Python interpreter. Phase 1D supersedes this document as the controlling remediation gate, but this Phase 1C lock remains unpassed.
...
| 32 duplicate clip keys | `BLOCKED` | The Phase 1C restricted audit confirmed 32 selected-cohort groups, all in `batch_000`. Zero had complete extracted-file hashes, so all 32 remained `OTHER_UNRESOLVED`; aggregate safety passed. Missing hashes are evidence unavailability, not proof of corruption or purge. | Phase 1D evidence-availability and v2 classification audit covering locator/hash/file survival, frame/extraction metadata, manifest payload, vector correspondence, and physical-source interpretation; deterministic resolution and post-resolution identity checks. | Historical merged clip/study embedding authority and all vision/fusion confirmatory input. |
...
| Raw alias, definition, and unit review | `BLOCKED` | OpenEvidence output cannot resolve project-specific metadata. The Phase 1C command used bare `python3`, bypassed the runbook's validated interpreter, and failed before packet creation when an older SCC system Python raised `future feature annotations is not defined`; no raw metadata were reviewed. | Resolve and validate Python >=3.10 without weakening the future import; rerun the restricted packet; then obtain targeted mechanical and clinician adjudication of aliases, definitions, units, methods/views/timing, indexing, and formula candidates. | Final feature masks, target units, native-unit margins, and final task dispositions. |
...
| Thresholds and margins | `PARTIAL` | Historical primary `<40`; secondary `<=40` and `<50`. The label-only threshold audit passed: 103/2,833 common labels and 20/426 common test labels equal exactly 40, so the inequality sensitivity is materially nontrivial. LVEF tolerance/margin/band values remain `EXPERT_INFERENCE`. | Preserve the completed count audit; verify LVEF unit/method mix. Non-LVEF native-unit margins remain unresolved until unit/definition review; explicit “no defensible margin” is acceptable. | Any non-LVEF margin/tie claim and final endpoint registry checksum; the exact-40 count itself is no longer blocking. |
...
| Aggregate safety | `PARTIAL` | Phase 1B and completed Phase 1C duplicate/threshold aggregate gates passed without identifier output. The later clinical command failed before producing a packet. | Each Phase 1D audit and eventual run must pass the allowlisted aggregate-only export gate; restricted roots must resolve outside Git. | Export of any new aggregate packet. |
